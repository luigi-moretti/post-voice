...
| Representação no banco | `'1'`/`'0'`, nunca booleano. `update_option()` pula a escrita quando o valor novo é igual ao atual, e opção inexistente lê como `false` — então gravar `false` num site que nunca abriu a tela não grava nada, e a leitura seguinte volta pro default "ligado". O site nunca conseguiria desligar. Coberto por teste. |
