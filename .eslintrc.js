module.exports = {
	// Este arquivo é a raiz do repositório para a busca de configuração do
	// ESLint: sem `root: true`, ESLint continua subindo diretórios acima
	// deste e pode encontrar outro `.eslintrc.js`, cada um resolvendo seu
	// próprio `@typescript-eslint` a partir de um `node_modules` diferente —
	// o que ESLint reporta como "couldn't determine the plugin uniquely" e
	// derruba tanto `lint:js` quanto o hook de pre-commit. `root: true` para
	// a busca aqui.
	root: true,
