...
- Nenhum import atravessa para outra feature; o que precisa ser compartilhado vai para `shared/`, a partir do segundo consumidor (ADR-0004, ADR-0005). **A ADR-0005 está `aceita-com-desvio`**: cinco imports cross-feature em TypeScript estão congelados em `desvios:` — inclusive entre `narration/editor` e `pronunciation/editor`. Não os desfaça de passagem; `npm run lint:arch` sabe quais são.
